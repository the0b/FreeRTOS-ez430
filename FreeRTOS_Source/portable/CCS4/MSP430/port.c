...
not the MCLK. */
#define portACLK_FREQUENCY_HZ			( ( portTickType ) 12000 )
...
void vPortSetupTimerInterrupt( void )
{
	...
	// Configure LFXT1 to run from internal very low power, low frequency oscillator (12 kHz)
	BCSCTL1 &= ~XTS;
	BCSCTL3 |= LFXT1S_2;

	// Run the timer off the ACLK.
	TACTL = TASSEL_1;
